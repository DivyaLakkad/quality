...
def do_something(filepath,download_path): 
    ...
    for x in range(i):
        ...
        newfilename_w =  str(diff_df['Project No.'].values[x])+ '-' + str(diff_df['Type of occurrence:'].values[x]) +'-'+ str(diff_df['Occurrence No.'].values[x]) + '.docx'
       
        
        document.save(download_path + newfilename_w) 
        convert(download_path + newfilename_w)
        newfilename = str(diff_df['Project No.'].values[x])+ '-'+ str(diff_df['Type of occurrence:'].values[x]) +'-' + str(diff_df['Occurrence No.'].values[x]) + '.pdf'
        newfilepath_w =  os.path.join(download_path, newfilename_w)
        newfilepath =  os.path.join(download_path, newfilename)
        ...
        # os.rename(filepath + r'\OLD_FILE.xlsx',filepath + r'\OLD_FILE_' + str(date)+ str(time)+ '.xlsx')
        # os.rename(filepath + r'\NEW_FILE.xlsx',filepath + r'\OLD_FILE.xlsx')
        # writer = pd.ExcelWriter('OLD_FILE_'+str(lastruntime) +'.xlsx', engine='xlsxwriter')
...
# s.run()
filepath = r'C:\Users\divyal\Desktop\projects\Info_file' #give path where the file is stored
download_path = 'C:\\Users\\divyal\\Desktop\\projects\\Quality_form_downlaod\\'
