...
def do_something(filepath, download_path):
    ...
    #new_list = []
        ...
        if diff_df.empty:
            return
        # my_date = diff_df['Report Date'].dt.strftime('%Y')
        # print (my_date)
        diff_df['Report Date'] = diff_df['Report Date'].dt.strftime('%d/%m/%Y')
        print(diff_df['Report Date'])
        ...
        for x in range(i):
            ...
            folder_name = folder_pre_name + "_"+ file_name
            length = len(output)
            print(length)
            ...
            employeename = diff_df['Name'].values[x]
            body = f"Hey {employeename}\n\nPlease find attached word document and pdf of {file_name} ({projnum} / {occurrence_num})\n\nCheers\nQuality Rocks"
